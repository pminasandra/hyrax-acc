...
def load_acc_file(csvfilepath, subset_cols=True):
    """
    ...
    csvfile["Timestamp"] += pd.to_timedelta(config.ACC_GPS_OFFSET, unit='s')
...
def load_acc_files(subset_cols=True):
    ...
    Args:
        subset_cols: whether to subset to specific cols
    Yields:
        2-tuple: accname (str), and csvfile (pd.DataFrame)
    Raises:
        AssertionError: if there are inappropriate csvfiles
    """
    tgtpath = os.path.join(config.ACC_DIR)
    for csvfilepath in glob.glob(os.path.join(tgtpath, "*.csv")):
        csvfile = load_acc_file(csvfilepath, subset_cols=subset_cols)
